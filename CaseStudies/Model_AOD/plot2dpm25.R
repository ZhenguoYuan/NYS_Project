...
gg_pm25_ori <- plot2d(data = pm25_ori, fill = pm25_ori$PM25_Pred, 
                  colorbar = jet.colors, colorbar_limits = c(3, 12),
                  shp = myshp, legend_name = 'PM2.5', title = paste(as.character(year), 'PM2.5'))
...
gg_pm25_gap <- plot2d(data = pm25_gap, fill = pm25_gap$PM25_Pred, 
                      colorbar = jet.colors, colorbar_limits = c(3, 12),
                      shp = myshp, legend_name = 'PM2.5', title = paste(as.character(year), 'PM2.5'))
...
gg_pm25_cld <- plot2d(data = pm25_cld, fill = pm25_cld$PM25_Pred, 
                      colorbar = jet.colors, colorbar_limits = c(3, 12),
                      shp = myshp, legend_name = 'PM2.5', title = paste(as.character(year), 'PM2.5'))

# ---------- Difference (Full - Original) ---------- #
pm25.diff <- data.frame(Lat = pm25$Lat, Lon = pm25$Lon, diff = pm25$PM25_Pred - pm25_ori$PM25_Pred)
pm25.diff <- cutByShp(shp.name = shp.name, pm25.diff)

gg.pm25.diff <- plot2d(data = pm25.diff, fill = pm25.diff$diff, 
                      colorbar = jet.colors, colorbar_limits = c(-2, 0.5),
                      shp = myshp, legend_name = 'PM2.5', title = paste(as.character(year), 'PM2.5 Diff'), 
                      xlim = c(-80, -71.6), ylim = c(40.5, 45.25))

# Stat
pm25.diff.stat <- pm25$PM25_Pred - pm25_ori$PM25_Pred
mean(pm25.diff.stat)
...
pm25_ori_pop$PM25.POP <- pm25_ori_pop$PM25_Pred * pm25_ori_pop$pop
...
gg_ratio <- plot2d(data = ratio, fill = ratio$r, 
                          colorbar = jet.colors, colorbar_limits = c(0.98, 1.02),
