...
# Notice: the size of saved image is 800 * 750
#
# Apr 6, 2018
#----------------------------------------------------------------------
...
gg <- ggmap(map, extent = "panel") + 
  ...
  geom_point(data = sites.uni, aes(x = lon, y = lat, colour = type, shape = type, size = type), alpha = 0.8) + 
  ...
  scale_fill_manual(values = c("#ffffff")) + # Set colors of geom_polygon
  scale_color_manual(values = c("#d84951", "#4357a3")) + # Set colors of geom_points
  scale_shape_manual(values = c(18, 17)) + # Set shapes of geom_points
  scale_size_manual(values = c(4, 3)) # Set sizes of geom_points

# ----- Adding Noth Arrow ----- #
north2(gg, x = 0.89, y = 0.85, symbol = 1, scale = 0.13)

png(file = '~/Downloads/map.png', width = 800, height = 750, units = 'px', res = 600)
