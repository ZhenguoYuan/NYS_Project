...
inpath <- paste('/home/jbi6/terra/MAIAC_GRID_OUTPUT/RF/', as.character(year), sep = '')
outpath <- paste('/home/jbi6/terra/MAIAC_GRID_OUTPUT/RF/Model_Perform/', as.character(year), sep = '')
...
for (i in 1 : numOfYear(year)) {
  ...
  snow.im1.aaot <- rbind(snow.im1.aaot, data.frame(rank = which(names(model.perform$importance1) == 'Snow_Cover_New'),
                                                   value = model.perform$importance1['Snow_Cover_New']))
  snow.im2.aaot <- rbind(snow.im2.aaot, data.frame(rank = which(names(model.perform$importance2) == 'Snow_Cover_New'),
                                                   value = model.perform$importance2['Snow_Cover_New']))
...
model.perform.aaot <- list(rsq = rsq.aaot, mse = mse.aaot, 
                           snow.im1 = snow.im1.aaot, snow.im2 = snow.im2.aaot, 
...
for (i in 1 : numOfYear(year)) {
  ...
  snow.im1.taot <- rbind(snow.im1.taot, data.frame(rank = which(names(model.perform$importance1) == 'Snow_Cover_New'),
                                                   value = model.perform$importance1['Snow_Cover_New']))
  snow.im2.taot <- rbind(snow.im2.taot, data.frame(rank = which(names(model.perform$importance2) == 'Snow_Cover_New'),
                                                   value = model.perform$importance2['Snow_Cover_New']))
...
model.perform.taot <- list(rsq = rsq.taot, mse = mse.taot, 
                           snow.im1 = snow.im1.taot, snow.im2 = snow.im2.taot, 
